...
final class ServerMessageReceiver(in: BufferedReader, client: ClientHandler) extends MessageReceiver{
  ...
  def readMessage = {
    while (!client.socket.isClosed) {
      ...
      if (jsonText != null) {
        ...
        matchType(JsonType.withName(jsonObject.optString("type")), jsonObject)
      }
    }
  }

  private def matchType(x: JsonType, jSONObject: JSONObject): Unit = x match {
